...
function spaceinvaders() {
    ...
    startLeftMove = keyObservable('keydown','ArrowLeft',()=>new Move(-.1)),
    startRightMove = keyObservable('keydown','ArrowRight',()=>new Move(.1)),
    ...
    const
    startAliens = [...Array(Constants.StartAlienCount)]
      .map((_,i)=>createCircle("alien")(i)
         (Constants.StartTime)(Constants.StartAlienRadius)(new Vec(Constants.CanvasSize/2, Constants.CanvasSize/3))
  ...
  handleCollisions = (s:State) => {
    const
      ...
      spawnChildren = (r:Body)=>
                            ...
                            ? [child(r,1), child(r,-1)] : [],
      newAliens = flatMap(collidedAliens, spawnChildren)
        .map((r,i)=>createCircle('alien')(s.objCount + i)(s.time)(r.radius)(r.pos)(r.vel)),
  ...
  tick = (s:State,elapsed:number) => {
    const 
      expired = (b:Body)=>(elapsed - b.createTime) > 230,
    ...
    return handleCollisions({...s, 
      ...
      aliens: s.aliens.map((o:Body) => <Body>{
        ...o,
        rotation: o.rotation + o.torque,
        angle:o.angle+o.rotation,
  ...
  function updateView(s: State) {
    ...
    attr(ship,{transform:`translate(${s.ship.pos.x + s.ship.angle},${s.ship.pos.y})`});
    // REMOVE????
